...
understands what's in them — who's there, the mood, the place, the occasion — and
...
AI has changed almost every category of software — it writes code, drafts
copy, edits video — but photo management is still stuck where it was a decade
...
- **Understands every photo on upload** — captions, faces, mood, location, and
  event type are extracted automatically, no manual tagging required.
...
- **One AI assistant that does all of it** — a conversational agent with
  tool access to the whole feature set above.
...
when a user later asks it a question. Nothing is computed on demand; it's all
pre-indexed at upload time so the assistant can respond in real time.

---

## The AI assistant

The assistant is a tool-using agent, not a single-shot chatbot. It runs a
turn-based loop (up to 5 turns per request) against OpenAI's function-calling
API, backed by a purpose-built set of tools that query the user's real data
directly:
...
the real result is handed back to the model → the model writes one short
reply summarizing what was found, while the app's UI renders the actual
photos.
...
a layer of indirection — and infrastructure — that this use case doesn't
need. The assistant already lives inside the same app as the data it's
querying, with direct, authenticated database access; going through MCP
here would be solving a multi-client, cross-service integration problem
that Gathrd doesn't have. It's a reasonable next step if Gathrd ever needs
to expose the photo library to *external* agents (e.g. letting Claude
Desktop query your library directly) — but that's a different product
surface than the in-app assistant.
