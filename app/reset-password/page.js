...
// ── Input Field ───────────────────────────────────────────────────────────────
function Field({ label, type, name, value, onChange, showToggle, toggled, onToggle }) {
  const [focused, setFocused] = useState(false);
  return (
    <div style={{ marginBottom:18 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:7 }}>
        <label htmlFor={`field-${name}`} style={{ fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:600, letterSpacing:'0.12em', textTransform:'uppercase', color: focused?'#111':'rgba(17,17,17,0.38)', transition:'color 0.2s' }}>
          {label}
        </label>
        {showToggle && (
          <button type="button" onClick={onToggle}
            style={{ fontSize:11, fontWeight:500, letterSpacing:'0.04em', color:'rgba(17,17,17,0.35)', background:'none', border:'none', cursor:'pointer', fontFamily:"'Syne',sans-serif", padding:0, lineHeight:1 }}
      ...
      <input id={`field-${name}`} type={type} name={name} value={value} onChange={onChange}
...
function ResetPasswordContent() {
  ...
  const passwordChecks = {
    ...
    matches: form.password.length > 0 && form.confirmPassword.length > 0 && form.password === form.confirmPassword,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token || !email) { setError('This reset link is invalid or incomplete.'); setLoading(false); return; }
    if (!form.password || !form.confirmPassword) { setError('Please enter and confirm your new password.'); setLoading(false); return; }
  ...
  return (
    <>
      ...
      <div className={loaded?'fi':''} style={{ opacity: loaded?undefined:0 }}><FloatingMemories /></div>
      ...
      <div style={{ position:'relative', zIndex:10, minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px' }}>
        ...
        <div className={loaded?'fu-1':''} style={{ opacity: loaded?undefined:0, marginBottom:28, textAlign:'center' }}>
          <a href="/" style={{ fontFamily:"'Syne',sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.05em', color:'#111', textDecoration:'none' }}>gathrd</a>
        </div>

        <div className={`card ${loaded?'fu-2':''}`} style={{ opacity: loaded?undefined:0 }}>
          <div style={{ marginBottom:30, textAlign:'center' }}>
            <h1 style={{ fontFamily:"'Instrument Serif',serif", fontSize:'clamp(26px,3vw,34px)', fontWeight:400, fontStyle:'italic', color:'#111', letterSpacing:'-0.02em', lineHeight:1.15, marginBottom:8 }}>Reset your password.</h1>
            <p style={{ fontFamily:"'Syne',sans-serif", fontSize:13, color:'rgba(17,17,17,0.38)', letterSpacing:'0.01em' }}>Choose a new password and get back to your memories</p>
          </div>

          {email && (
            <div className={loaded?'fu-3':''} style={{ opacity: loaded?undefined:0, marginBottom:18, background:'rgba(17,17,17,0.03)', border:'1.5px solid rgba(17,17,17,0.08)', color:'rgba(17,17,17,0.55)', padding:'12px 14px', borderRadius:10, fontFamily:"'Syne',sans-serif", fontSize:12, textAlign:'center', wordBreak:'break-word' }}>
              Resetting account: <span style={{ color:'#111', fontWeight:600 }}>{email}</span>
            </div>
          )}

          {error && <div style={{ background:'rgba(220,38,38,0.06)', border:'1.5px solid rgba(220,38,38,0.18)', color:'#dc2626', padding:'11px 14px', borderRadius:10, fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:500, marginBottom:20, textAlign:'center' }}>{error}</div>}
          {success && <div style={{ background:'rgba(22,163,74,0.06)', border:'1.5px solid rgba(22,163,74,0.18)', color:'#15803d', padding:'11px 14px', borderRadius:10, fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:500, marginBottom:20, textAlign:'center' }}>{success}</div>}

          {invalidLink ? (
            <div className={loaded?'fu-4':''} style={{ opacity: loaded?undefined:0 }}>
              <div style={{ background:'rgba(17,17,17,0.03)', border:'1.5px solid rgba(17,17,17,0.08)', color:'rgba(17,17,17,0.6)', padding:'14px 16px', borderRadius:10, fontFamily:"'Syne',sans-serif", fontSize:13, marginBottom:20, textAlign:'center', lineHeight:1.6 }}>
                This reset link is invalid, incomplete, or expired.
              </div>
              <button type="button" className="primary-btn" onClick={() => router.push('/forgot-password')}>Request new link →</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className={loaded?'fu-3':''} style={{ opacity: loaded?undefined:0 }}>
                <Field label="New Password" type={showPassword?'text':'password'} name="password" value={form.password} onChange={handleChange} showToggle toggled={showPassword} onToggle={() => setShowPassword(p => !p)} />
              </div>
              <div className={loaded?'fu-4':''} style={{ opacity: loaded?undefined:0 }}>
                <Field label="Confirm Password" type={showConfirmPassword?'text':'password'} name="confirmPassword" value={form.confirmPassword} onChange={handleChange} showToggle toggled={showConfirmPassword} onToggle={() => setShowConfirmPassword(p => !p)} />
              </div>
              <div style={{ marginBottom:14, marginTop:8, background:'rgba(17,17,17,0.03)', border:'1.5px solid rgba(17,17,17,0.08)', borderRadius:10, padding:'12px 14px' }}>
                <div style={{ fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(17,17,17,0.35)', marginBottom:8 }}>Password checks</div>
                <div style={{ display:'grid', gap:6 }}>
                  <div style={{ fontFamily:"'Syne',sans-serif", fontSize:12, color: passwordChecks.minLength?'#15803d':'rgba(17,17,17,0.42)' }}>{passwordChecks.minLength?'✓':'•'} At least 6 characters</div>
                  <div style={{ fontFamily:"'Syne',sans-serif", fontSize:12, color: passwordChecks.matches?'#15803d':'rgba(17,17,17,0.42)' }}>{passwordChecks.matches?'✓':'•'} Passwords match</div>
                </div>
              </div>
        ...
        <div style={{ marginTop:24, textAlign:'center', fontFamily:"'Syne',sans-serif", fontSize:13, color:'rgba(17,17,17,0.38)' }}>
          ...
          <a href="/login" style={{ color:'#111', fontWeight:700, textDecoration:'none', borderBottom:'1.5px solid rgba(17,17,17,0.22)', paddingBottom:1, transition:'border-color 0.2s' }}
