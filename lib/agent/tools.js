...
Returns photo metadata — the actual zip is built client-side after this response.`,
  ...
  function: {
    name: "get_top_photos",
    description: "Get the highest-scoring photos from the user's library using the content score system. Use when user asks for 'best photos', 'top picks', 'highlight worthy', 'most shareable'. Returns photos ranked by score.",
    parameters: {
      type: "object",
      properties: {
        min_score: { type: "number", description: "Minimum content score 0–100. Default 70." },
        limit:     { type: "number", description: "Max photos to return. Default 20." },
        days_ago:  { type: "number", description: "Limit to last N days. Omit for all-time." },
      },
    },
  },
},
{
  ...
  function: {
    ...
    description: "Generate platform-optimized social media captions for a set of photos. Use when user asks to 'write captions', 'create a week of content', 'draft posts'. Returns one caption per photo.",
    parameters: {
      type: "object",
      properties: {
        photo_ids: { type: "array", items: { type: "number" }, description: "Photo IDs to caption" },
        platform:  { type: "string", enum: ["instagram", "linkedin", "twitter", "threads"], description: "Target platform" },
      },
      required: ["photo_ids"],
    },
  },
